...
class Observation_Label(object):
    def __init__(self):
        ...
        self.obs_dir = 'observation16'

        # Load images
        #self.images_path = glob.glob(self.obs_dir + '/*.jpg')
        self.images_path = glob.glob('observation16/*.jpg')
        # process annotation
        #annotation =  json.load(open(self.obs_dir + '/train_region.json'))
        annotation =  json.load(open('observation16/train_region.json'))
        ...
        self.obs_dict = {}
        self.obj_trial_count = {(i+1):0 for i in range(NUM_TYPE)}
    ...
    def label_obs(self):
        for a in self.annotation:
            ...
            image_path = os.path.join('observation16', a['filename'])
            ...
            if type_key not in self.obs_dict.keys():
                self.obs_dict[type_key] = {i:np.zeros(NUM_TYPE) for i in range(self.occl_bin)}
            ...
            for idx, occl in enumerate(self.occl_list):
                ...
                for r in occl_ratios:
                    ...
                    if len(result['distribution']) == 0:
                        dist = np.ones(NUM_TYPE) / NUM_TYPE * 1.0
                    else:
        ...
        with open('observation_model_16.json', 'w') as f:
